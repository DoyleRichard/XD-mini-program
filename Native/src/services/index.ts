...
export type IServiceAppInfo = { appName: string; logo: string }

// 模拟调用微信开放平台接口，获取应用信息
export function getMiniAppInfo(appId: string): Promise<IServiceAppInfo> {
	return new Promise(resolve => {
		const _appId = appId as keyof typeof appInfo
		resolve(appInfo[_appId])
	})
}
